...
usage( gchar *appname )
{
    g_printf( "  %s { -f <filename> | svm-ip service-group [ options | source-id ] }\n\n", appname );
    g_printf( "    sourceId becomes optional and not used if -s or --srcids options are specified.\n" );
    g_printf( "    options:\n" );
    g_printf( "      -b  | --stbbase number        the base address for a range of settops\n" );
    g_printf( "                                    utilizes the second mac octet, 1 to 255 - default 1.\n" );
    g_printf( "      -c  | --stbcnt number         the number of settops to simulate\n" );
    g_printf( "                                    utilizes last 4 mac octets; default 1.\n" );
//    g_printf( "      -d  | --dwell number          the tuned channel dwell time milliseconds - default 250.\n" );
    g_printf( "      -f  | --file filename         a configuration file specifing multiple cli invocations\n" );
    g_printf( "      -h  | --hold timems           hold off time before task is started. ie inter-task gap\n" );
    g_printf( "      -l  | --loop                  round robin the stb and srcids.\n" );
    g_printf( "      -n  | --noinit                skip stb initialization.\n" );
    g_printf( "      -r  | --rate timems           milliseconds between channel changes - default 250.\n" );
    g_printf( "      -s  | --srcids number number  range of source IDs begining to ending.\n" );
    g_printf( "      -z  seconds                   dwell on channel for n seconds.\n" );
    g_printf( "      -vi                           verbose dsmcc input display.\n" );
    g_printf( "      -vo                           verbose dsmcc output display.\n" );
    g_printf( "      -ve                           verbose dsmcc error display.\n" );
    g_printf( "      -vf                           verbose dsmcc failure display.\n" );
    g_printf( "      -vs                           verbose stb fsm display.\n" );
    g_printf( "      -vx                           verbose stb fsm abnormal display.\n" );
    g_printf( "      -vd                           dump stb info on termination.\n" );
//    g_printf( "\n\n     the channel change rate will cause a backlog of requests if stb count is large\n" );
//    g_printf( "       and / or dwell is too small.\n" );
    ...
    g_printf( "       i.e. cli sdv4 922 -b 2 -c 80 - r 400 -s 64001 64500 -n -l.\n\n" );
}
...
init_and_parse_cli( gint argcnt, gchar *argstrs[], int sgndx )
{
    ...
    g_printf( "   " );
    for ( i = 0; i < argcnt; i++ )
        g_printf( " %s", argstrs[i] );
            
    opt_parm = 2;
    for ( i = 1; i < argcnt; i++ )
    {
        //g_printf( "ndx.i = %i.%i %s\n", sgndx, i, argstrs[i] );
        ...
        {   /* here if server is defined, no opt and no service group */
            ...
            {
                g_printf( "service group is illegal value (%i)\n",
        ...
        {
            g_printf( "processing error in command line args\n" );
        }
    }
    
    if ( sgs[sgndx].srcidmin == NOSRCID || sgs[sgndx].srcidmax == NOSRCID )
    {
        g_printf( ">>> No sourceId(s)\n\n" );
        return FALSE;
    }
            g_printf( "  validated\n" );
...
main( gint argcnt, char *argstrs[] )
{
    ...
    g_printf( "\n  %s version %s.\n", appname, version );
    g_printf( "  Charles Donaldson charlesdonaldson@motorola.com.\n\n" );
    ...
    for ( i = 0; i < sgcount; i++ )
    {
        ...
        {
            g_printf( "service group init failed\n" );
            exit (1);
        }
    }
    ...
    for ( i = 0; i < sgcount; i++ )
    {
        for ( j = 0; j < sgs[i].stbcnt; j++ )
        {
            ...
            {
                g_printf( "server timeout failure on: " );
